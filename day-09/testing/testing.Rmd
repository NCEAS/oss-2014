...
1. Add tests for the functions we've outlined from the previous section.
2. create a folder called `inst/tests` in the package folder.
3. save tests into a single R script with the word `test` in the title e.g. `test.R`. 
4. Group tests where necessary using `context("what the context is")`.
5. Run tests from the R prompt using `test_dir()` or `test_file()` from the `testthat` package. Or if you have `devtools` loaded, then simply run `test()` 
